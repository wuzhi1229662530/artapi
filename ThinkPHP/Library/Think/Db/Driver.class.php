...
abstract class Driver {
    ...
    public function query($str,$fetchSql=false) {
        ...
        //print_r($_SERVER);
        $folder_name = date("Ym");
        $file_name = "/sql_".date("Ymd").".log";

        $log =  "./Logs/".$folder_name;
        $_log =   "./Logs/".$folder_name.$file_name;
        
        if (!is_dir($log)) {
            mkdir($log,0777, true);
        }
        
        if($_SERVER['HTTP_TYPE'] == '2'){
            $type = 'IOS';
        }else if($_SERVER['HTTP_TYPE'] == '1'){
            $type = 'Android';
        }else{
            $type = 'Other';
        }

        file_put_contents($_log, date("Y-m-d H:i:s")." ".$_SERVER['PATH_INFO']." ".$type." ".$str.PHP_EOL, FILE_APPEND);
        //添加log结束
